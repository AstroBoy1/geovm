...
def main():
    ...
    statement = "INSERT INTO metadata.url (id, url) SELECT id, CONCAT('https://farm', flickr_farm, '.staticflicker.com/', flickr_server, '/', id, '_', flickr_secret, '.jpg') FROM metadata.imagemetadata LIMIT 10"    
    cursor.execute(statement)
    cnx.commit()
    # df = pd.read_sql("SELECT t1.id, t1.url FROM metadata.url AS t1, metadata.imagemetadata AS t2 where t1.id = t2.id", con=cnx)
    # print("URL Table", df)
    df = pd.read_sql("SELECT * FROM metadata.url", con=cnx)
    print("URL Table", df)
    cnx.close()
